...
// DOM 요소 - GCS 게임 관리
const gamePlatformSelect = document.getElementById("game-platform-select")
...
// DOM 요소 - Firestore 게임 관리
const fsGamePlatformSelect = document.getElementById("fs-game-platform-select")
...
const fsGameDetailsContainer = document.getElementById("fs-game-details-container")
const fsGameDetails = document.getElementById("fs-game-details")
const fsGameEditForm = document.getElementById("fs-game-edit-form")
...
function selectGame(index) {
  ...
  currentGame = filteredGames[index]
  currentGameCode = currentGame.rj_code || ""
  currentPlatform = gamePlatformSelect.value
...
function renderGameDetails(game) {
  ...
  gameDetails.innerHTML = `
    ...
    <div class="game-detail-actions">
      <button class="edit-btn" data-rjcode="${rjCode}">수정</button>
      <button class="delete-btn" data-rjcode="${rjCode}">삭제</button>
    </div>
  `

  // 수정 버튼 이벤트 리스너 추가
  const editBtn = gameDetails.querySelector(".edit-btn")
  if (editBtn) {
...
function showEditForm() {
  ...
  gameEditForm.style.display = "block"
  isEditMode = true
}
...
function hideEditForm() {
  ...
  gameEditForm.style.display = "none"
  isEditMode = false
}
...
async function saveGame() {
  ...
  currentGame.primary_tag = gamePrimaryTag.value
  ...
  try {
    ...
    // 저장한 게임 선택
    const savedGameIndex = filteredGames.findIndex((game) => game.rj_code === currentGameCode)
    if (savedGameIndex !== -1) {
      selectGame(savedGameIndex)
    }
  } catch (error) {
...
function showDeleteGameConfirmModal(rjCode, title) {
  ...
  confirmDeleteGameBtn.dataset.source = "gcs"
}
...
deleteAllGamesBtn.addEventListener("click", () => {
  ...
  confirmDeleteAllGamesBtn.dataset.source = "gcs"
})
...
// 편집 취소 버튼
fsCancelEditBtn.addEventListener("click", () => {
  hideFsEditForm()
})

// 게임 목록 불러오기
async function loadFsGameList() {
  const platform = fsGamePlatformSelect.value
...
function selectFsGame(index) {
  ...
  fsCurrentGame = fsFiltredGames[index]
  fsCurrentGameCode = fsCurrentGame.rj_code || ""
  fsCurrentPlatform = fsGamePlatformSelect.value

  // 게임 상세 정보 표시
  renderFsGameDetails(fsCurrentGame)
  hideFsEditForm()
}
...
function renderFsGameDetails(game) {
  ...
  // 수정 버튼 이벤트 리스너 추가
  const editBtn = fsGameDetails.querySelector(".edit-btn")
  if (editBtn) {
    editBtn.addEventListener("click", () => {
      showFsEditForm()
    })
...
function showFsEditForm() {
  ...
  // 상세 정보 숨기고 수정 폼 표시
  fsGameDetails.style.display = "none"
  fsGameEditForm.style.display = "block"
  fsIsEditMode = true
}

// 수정 폼 숨기기
function hideFsEditForm() {
  fsGameDetails.style.display = "block"
  fsGameEditForm.style.display = "none"
  fsIsEditMode = false
}
...
async function saveFsGame() {
  ...
  fsCurrentGame.primary_tag = fsGamePrimaryTag.value
  ...
  try {
    ...
    // 게임 목록 다시 불러오기
    await loadFsGameList()

    // 저장한 게임 선택
    const savedGameIndex = fsFiltredGames.findIndex((game) => game.rj_code === fsCurrentGameCode)
    if (savedGameIndex !== -1) {
      selectFsGame(savedGameIndex)
    }
  } catch (error) {
    console.error("게임 저장 오류:", error)
    showToast("오류", "게임 데이터 저장에 실패했습니다. 다시 시도해주세요.", "error")
  ...
  tagStats.forEach((item) => {
    ...
    html += `
      <div class="tag-stats-item">
        <div class="tag-stats-name">${item.tag}</div>
        <div class="tag-stats-bar-container">
          <div class="tag-stats-bar">
            <div class="tag-stats-bar-fill" style="width: ${percentage}%"></div>
          </div>
        </div>
        <div class="tag-stats-count">${item.count}</div>
      </div>
    `
  })
