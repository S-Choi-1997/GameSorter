            ...
            def process_batch(batch_idx, batch_files, batch_enhanced):
                try:
                    ...
                    for i, (original, enhanced) in enumerate(zip(batch_files, batch_enhanced)):
                        ...
                        file_prompts.append(
                            f"{i+1}. 원본파일명: {original}, RJ코드: {rj_code}, 제목: {title}, 태그: {tags}"
                        )
                    logging.info(f"Batch {batch_idx + 1}: Sending to GPT: {file_prompts}")
                    ...
                    prompt_text = (
                        ...
                        "   ⚠️ 절대 [RJ없음], [RJ코드], [분류없음], Rj와 숫자가 아닌 태그를 사용하지 마세요. 무조건 [기타]로 통일하세요.\n"
                        "3. 두 번째 태그는 장르 키워드를 넣습니다:\n"
                        "   - [청아], [순애], [NTR], [RPG] 등 장르적 키워드가 제목이나 태그에 명확히 드러나는 경우 사용합니다.\n"
                        "   - 빼앗는다, 유혹한다 같은 표현은 NTR로 간주하지 마세요.\n"
                        "   - 장르를 판단하기 어려운 경우 [기타]로 표기합니다.\n"
                        "4. 제목 정리 규칙:\n"
                        "   - 파일명이나 정보에 이미 한국어 제목이 포함된 경우, 해당 한국어 부분을 우선 사용하세요.\n"
                        "   - 일본어 제목만 있는 경우, 반드시 한국어로 번역하세요(고유명사는 그대로 유지).\n"
                        "     예: むすめせいかつ → 딸의생활\n"
                        "     예: ☆特別なApp☆ → ☆특별한App☆\n"
                        "   - 제목은 직역 중심으로, 원문 의미를 살려 자연스럽게 다듬으세요.\n"
            ...
            def process_batch(batch_idx, batch_indices):
                try:
                    ...
                    prompt_text = (
                        ...
                        "   - 그 외: [기타].\n"
                        "   ⚠️ [RJ없음], [분류없음] 같은 Rj와 숫자가 아닌 태그는 절대 사용하지 마세요. [기타]로 통일하세요.\n"
                        "3. 두 번째 태그는 장르 키워드:\n"
...
class MainWindow(QMainWindow):
    ...
    def get_dlsite_data(self, rj_code):
        ...
        for attempt in range(3):
            try:
                ...
                title = title_tag.text.strip() if title_tag else rj_code
    ...
    def analyze_with_ai(self):
        ...
        for i, f in enumerate(files):
            ...
            enhanced_files.append(data or {'rj_code': None, 'title': f, 'tags': [], 'engine': '기타', 'maker': ''})
